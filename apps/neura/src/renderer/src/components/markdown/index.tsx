  ...
  return (
    <ReactMarkdown
      components={{
        ...
        blockquote: ({ children }) => (
          <blockquote className="border-l-4 border-blue-400/70 pl-4 py-2 mb-3 bg-muted/40 italic text-muted-foreground">
            {children}
          </blockquote>
        ),
        ...
        code: ({ children, className }) => {
          if (!className) {
            return (
              <code className="bg-muted text-red-300 px-1.5 py-0.5 rounded font-mono break-words">
                {children}
              </code>
            );
          }
        ...
        pre: ({ children }) => (
          <pre className="bg-muted/60 border border-border rounded-md p-3 mb-3 overflow-x-auto whitespace-pre-wrap">
        ...
        th: ({ children }) => (
          <th className="border-b border-border px-1 py-3 font-semibold">
            {children}
          </th>
        ),
        td: ({ children }) => (
          <td className="border-b border-border px-1 py-3">{children}</td>
        ),
      }}
    >
      {children}
