...
import { registerIpcMain } from '@neura-desktop/electron-ipc/main';
...
const loadDevDebugTools = async () => {
  ...
  import('electron-devtools-installer')
    .then(({ default: installExtensionDefault, REACT_DEVELOPER_TOOLS }) => {
      const installer = installExtensionDefault as unknown as
        | typeof installExtensionDefault
        | { default?: typeof installExtensionDefault };
      const installExtension =
        typeof installer === 'function'
          ? installer
          : (installer as { default?: typeof installExtensionDefault })
              ?.default;

      if (typeof installExtension !== 'function') {
        logger.warn('Skipping React DevTools install: installer unavailable');
        return;
      }
