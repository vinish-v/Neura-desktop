...
| `visual-grounding` | Operation capabilities based on GUI Agent ([Neura](https://seed-tars.com/) / [Doubao 1.5 VL](https://www.volcengine.com/docs/82379/1554521)), without DOM-related tools. |
...
In Agent TARS's early preview version, we only supported Claude 3.7, but we still received numerous requests from the community for compatibility with various models. We initiated a Discussion about [Model Compatibility #377](https://github.com/bytedance/Neura-desktop/discussions/377), but due to architecture limitations at that time, we couldn't quickly achieve good model compatibility.
...
This design was first introduced in the release of [Neura 1.5](https://seed-tars.com/1.5/). Starting from Agent TARS Beta, Agent TARS supports GUI Grounding, and the process includes "real-time mouse tracking":
...
For Agent TARS, this is a very valuable beginning. From the earliest Neura-desktop, GUI Agent was the only first-class citizen, but now, GUI Agent is no longer an isolated entity, playing its role in an integrated environment!
