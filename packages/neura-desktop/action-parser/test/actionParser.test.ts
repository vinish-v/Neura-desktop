...
Action: click(start_box='[0, 964, 10, 984]')`;
  ...
  describe('native tools', () => {
    it('should parse file, browser extraction, process, and monitor actions', () => {
      const input = `Thought: Use native tools.
Action: write_file(path='C:\\tmp\\note.md', content='hello', overwrite='false')

extract_page(format='tables')

start_process(command='npm run dev', cwd='D:\\app')

create_monitor(url='https://example.com', interval_minutes='5', watch='page', query='', notify_on='change')`;

      const result = parseActionVlm(input);

      expect(result.map((item) => item.action_type)).toEqual([
        'write_file',
        'extract_page',
        'start_process',
        'create_monitor',
      ]);
      expect(result[0].action_inputs.path).toBe('C:\\tmp\\note.md');
      expect(result[1].action_inputs.format).toBe('tables');
      expect(result[2].action_inputs.command).toBe('npm run dev');
      expect(result[3].action_inputs.url).toBe('https://example.com');
    });
  });
});
