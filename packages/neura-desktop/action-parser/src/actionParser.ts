...
export function parseActionVlm(
  ...
  if (
    modelVer === NeuraModelVersion.V1_5 &&
  ...
  for (const rawStr of allActions) {
    const cleanedRawStr = rawStr.replace(/```[a-zA-Z]*|```/g, '').trim();
    if (!cleanedRawStr) {
      continue;
    }

    const normalizedActionStr = extractFirstFunctionCall(
      cleanedRawStr.replace(/\n/g, String.raw`\n`).trimStart(),
    );
    const actionInstance = parseAction(normalizedActionStr);
    let actionType = '';
    ...
    if (actionInstance) {
      ...
      for (const [paramName, param] of Object.entries(params)) {
        ...
        if (paramName.includes('start_box') || paramName.includes('end_box')) {
          ...
          if (screenContext?.width && screenContext?.height) {
            ...
            actionInputs[boxKey] = [x1, y1, x2, y2].every(Number.isFinite)
              ? [
                  (Math.round(
      ...
      const fallback = parseNaturalLanguageAction(cleanedRawStr);
      if (fallback) {
        actionType = fallback.action_type;
        actionInputs = fallback.action_inputs;
      }
    }
...
function extractFirstFunctionCall(actionStr: string): string {
  const text = actionStr.trim();
  const startMatch = text.match(/\b\w+\s*\(/);
...
function parseNaturalLanguageAction(
  actionStr: string,
): Pick<PredictionParsed, 'action_type' | 'action_inputs'> | null {
  ...
  const jsonTextMatch = normalized.match(
    /"type"\s*:\s*"text"[\s\S]*?"text"\s*:\s*"([^"]+)"/i,
  );
  if (jsonTextMatch?.[1]) {
    return {
      action_type: 'type',
      action_inputs: { content: jsonTextMatch[1] },
    };
  }

  const pyautoguiWriteMatch =
    normalized.match(
      /pyautogui\.write\(\s*message=['"`]{1,3}([\s\S]+?)['"`]{1,3}\s*\)/i,
    ) ||
    normalized.match(
      /pyautogui\.write\(\s*['"`]{1,3}([\s\S]+?)['"`]{1,3}\s*\)/i,
    );

  if (pyautoguiWriteMatch?.[1]) {
    return {
      action_type: 'type',
      action_inputs: { content: pyautoguiWriteMatch[1] },
    };
  }

  const typeMatch =
    normalized.match(/\btype\s+["“”']([^"“”']+)["“”']/i) ||
    normalized.match(/\benter\s+["“”']([^"“”']+)["“”']/i) ||
    normalized.match(/\binput\s+["“”']([^"“”']+)["“”']/i);

  if (typeMatch?.[1]) {
    const shouldSubmit =
      /\b(search|submit|press\s+enter|hit\s+enter|execute)\b/i.test(
        normalized,
      );
    return {
      action_type: 'type',
      action_inputs: {
        content: shouldSubmit ? `${typeMatch[1]}\\n` : typeMatch[1],
      },
    };
  }

  const navigateMatch =
    normalized.match(/\bnavigate\s+(?:to\s+)?([a-z0-9.-]+\.[a-z]{2,})(?:\b|\/)/i) ||
    normalized.match(/\bgo\s+to\s+([a-z0-9.-]+\.[a-z]{2,})(?:\b|\/)/i) ||
    normalized.match(/\bopen\s+([a-z0-9.-]+\.[a-z]{2,})(?:\b|\/)/i);

  if (navigateMatch?.[1]) {
    return {
      action_type: 'navigate',
      action_inputs: { content: navigateMatch[1] },
    };
  }

  if (/\b(press|hit)\s+enter\b/i.test(normalized)) {
    return {
      action_type: 'hotkey',
  ...
  if (/\b(finished|done|complete|completed)\b/i.test(lower)) {
    return {
