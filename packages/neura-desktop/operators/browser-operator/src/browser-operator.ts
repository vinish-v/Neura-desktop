...
import { Operator, parseBoxToScreenCoords } from '@neura-desktop/sdk/core';
...
type NativeBrowserActionInputs = Partial<{
  format: string;
  url: string;
  output_path: string;
  path: string;
...
export class BrowserOperator extends Operator {
  ...
  private readonly domElementAttribute = 'data-neura-element-id';
  ...
  public async screenshot(): Promise<ScreenshotOutput> {
    ...
    try {
      const domText = await this.captureDomMap(page);
  ...
  public async execute(params: ExecuteParams): Promise<ExecuteOutput> {
    ...
    const deviceScaleFactor = await this.getDeviceScaleFactor();
    const { x: startX, y: startY } = this.getMouseCoords({
      inputs: action_inputs,
      boxKey: 'start_box',
      coordsKey: 'start_coords',
      screenWidth,
      screenHeight,
      deviceScaleFactor,
    });
    ...
    try {
      ...
      switch (action_type) {
        ...
        case 'click_element':
          await this.handleClickElement(action_inputs);
          break;

        case 'type_element':
          await this.handleTypeElement(action_inputs);
        ...
        case 'left_single':
          if (Number.isFinite(startX) && Number.isFinite(startY)) {
            ...
            throw new Error(`Missing startX(${startX}) or startY${startX}.`);
          break;
        ...
        case 'extract_page':
          return await this.handleExtractPage(action_inputs);

        case 'download_url':
          return await this.handleDownloadUrl(action_inputs);

        case 'save_page_screenshot':
          return await this.handleSavePageScreenshot(action_inputs);
        ...
        case 'finished':
          if (this.options.onFinalAnswer) {
            const finalAnswer =
              action_inputs.content?.trim() || parsedPrediction.thought;
            if (finalAnswer) {
              await this.options.onFinalAnswer(finalAnswer);
            }
          }
  ...
  private async handleExtractPage(
    inputs: NativeBrowserActionInputs,
    ...
    return {
      status: END_STATUS,
      message: `Extracted current page as ${format}:\n\n${this.truncateExtraction(outputByFormat[format] || markdown)}`,
    };
  }
  ...
  private async handleDownloadUrl(
    ...
    const url = this.requireInput(inputs.url, 'url');
    const outputPath = this.resolveOutputPath(inputs.output_path);
    await this.assertCanWrite(outputPath, this.asBool(inputs.overwrite));
    ...
    return {
      status: END_STATUS,
      message: `Downloaded ${url} to ${outputPath}`,
    };
  }

  private async handleSavePageScreenshot(
    inputs: NativeBrowserActionInputs,
  ): Promise<ExecuteOutput> {
    const page = await this.getActivePage();
    const outputPath = this.resolveOutputPath(inputs.path);
    await this.assertCanWrite(outputPath, this.asBool(inputs.overwrite));
    await page.screenshot({
      path: outputPath,
      fullPage: this.asBool(inputs.full_page),
    });
    return {
      status: END_STATUS,
  ...
  private truncateExtraction(value: string, max = 16_000) {
  ...
  private requireInput(value: string | undefined, name: string) {
  ...
  private resolveOutputPath(rawPath: string | undefined) {
    return path.resolve(this.requireInput(rawPath, 'output_path'));
  }

  private async assertCanWrite(outputPath: string, overwrite = false) {
  ...
  private getMouseCoords({
    ...
    boxKey: 'start_box' | 'end_box';
    coordsKey: 'start_coords' | 'end_coords';
    screenWidth: number;
    screenHeight: number;
    deviceScaleFactor: number;
  }): { x: number | null; y: number | null } {
    const parsedCoords = inputs?.[coordsKey];
    if (
      Array.isArray(parsedCoords) &&
      Number.isFinite(parsedCoords[0]) &&
      Number.isFinite(parsedCoords[1])
    ) {
      return {
        x: parsedCoords[0] / deviceScaleFactor,
        y: parsedCoords[1] / deviceScaleFactor,
      };
    }

    const boxStr = inputs?.[boxKey] || '';
  ...
  private getElementSelector(elementId: string): string {
    if (!/^[a-zA-Z0-9_-]+$/.test(elementId)) {
      throw new Error(`Invalid element_id: ${elementId}`);
    }
    return `[${this.domElementAttribute}="${elementId}"]`;
  }

  private async handleClickElement(inputs: Record<string, any>) {
    const page = await this.getActivePage();
    const elementId = inputs.element_id || inputs.id;
    if (!elementId) {
      throw new Error('No element_id specified for click_element');
    }

    const selector = this.getElementSelector(elementId);
    const element = await page.$(selector);
    if (!element) {
      throw new Error(`Element not found for click_element: ${elementId}`);
    }
    ...
    if (box) {
      await this.uiHelper?.showClickIndicator(
        box.x + box.width / 2,
        box.y + box.height / 2,
      );
      await this.delay(150);
  ...
  private async handleTypeElement(inputs: Record<string, any>) {
    const page = await this.getActivePage();
    const elementId = inputs.element_id || inputs.id;
    if (!elementId) {
      throw new Error('No element_id specified for type_element');
    }
    if (!inputs.content) {
      throw new Error('No content specified for type_element');
    }

    const selector = this.getElementSelector(elementId);
    const element = await page.$(selector);
    if (!element) {
      throw new Error(`Element not found for type_element: ${elementId}`);
    }
    ...
    await this.delay(150);
    await element.click();
    await this.handleType({ content: inputs.content });
  }
  ...
  private async handleDrag(
    ...
    if (
      (!inputs.start_box && !inputs.start_coords) ||
      (!inputs.end_box && !inputs.end_coords)
    ) {
    ...
    const endCoords = this.getMouseCoords({
      inputs,
      boxKey: 'end_box',
      coordsKey: 'end_coords',
      screenWidth,
      screenHeight,
      deviceScaleFactor,
    });
  ...
  private async captureDomMap(page: Page): Promise<string | undefined> {
    try {
      const attributeName = this.domElementAttribute;
      const pageSnapshot = await page.evaluate((attr) => {
        const selectors = [
          'a[href]',
          'button',
          'input',
        ...
        const cleanLongText = (value: string | null | undefined) =>
        ...
        const captureVisibleText = () => {
          ...
          const walker = document.createTreeWalker(root, NodeFilter.SHOW_TEXT, {
            acceptNode(node) {
              const text = cleanLongText(node.textContent);
              if (text.length < 2) {
          ...
          while (walker.nextNode() && length < 5000) {
            const text = cleanLongText(walker.currentNode.textContent);
        ...
        const elements = Array.from(document.querySelectorAll(selectors))
          ...
          .map(({ el, rect }, index) => {
            ...
            const role = el.getAttribute('role') || inputEl.type || tagName;
            const label =
              cleanText(el.getAttribute('aria-label')) ||
              cleanText(el.getAttribute('placeholder')) ||
              cleanText(el.getAttribute('title')) ||
              cleanText(inputEl.value) ||
              cleanText(htmlEl.innerText) ||
              cleanText(el.textContent) ||
              cleanText(inputEl.name) ||
              cleanText(inputEl.id);
        ...
        return {
          ...
          title: document.title,
          pageText: captureVisibleText(),
          elements,
        };
      }, attributeName);

      if (
        !pageSnapshot.elements.length &&
      ...
      const lines = pageSnapshot.elements.map(
        (item) =>
          `${item.id} | ${item.role} | "${item.label}" | x=${item.x} y=${item.y} w=${item.w} h=${item.h}`,
      );

      const sections = [
        '## Browser Page',
        `URL: ${pageSnapshot.url || '(unknown)'}`,
        `Title: ${pageSnapshot.title || '(untitled)'}`,
        '',
        '## Visible Page Text',
        pageSnapshot.pageText || '(No readable visible page text captured.)',
      ];
      ...
      if (lines.length) {
        sections.push(
          '',
          '## Browser DOM Map',
          "Prefer these browser actions when possible: click_element(element_id='e0') or type_element(element_id='e1', content='text\\n').",
          'Visible elements:',
          ...lines,
        );
      }
      ...
      this.logger.warn('Failed to capture browser DOM map:', error);
      return undefined;
    }
  }
...
export class DefaultBrowserOperator extends BrowserOperator {
  ...
  public static async getInstance(
    ...
    if (!isCallUser) {
      ...
      this.logger.info(`Opening browser start page: ${targetUrl}`);
      await openingPage
        ?.goto(targetUrl, {
        ...
        .catch((error) => {
          this.logger.warn(
            'Opening browser start page timed out/failed:',
            error,
          );
        });
    }
