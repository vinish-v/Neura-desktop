...
import { IMAGE_PLACEHOLDER, MAX_IMAGE_LENGTH } from '@neura-desktop/shared/constants';
import { Conversation, Message } from '@neura-desktop/shared/types';
          ...
          content: [
            {
              type: 'text',
              text:
                'This is the current screen for the active task. Neura can control it through the provided Action Space. Return only the next executable step in exactly this format: Thought: one short sentence\\nAction: function_call(...). Do not describe the page. For browser pages, prefer DOM element actions from the Browser DOM Map over coordinate clicks whenever an element id is available.' +
                domText,
            },
            {
              type: 'image_url',
